...
While the fields are represented by a set of 2D cylindrical grids, the macroparticles can move in three-dimensional space as shown by Figure 1. Generally, these macroparticles are initialized uniformly along the :math:`r,\phi,z` directions; however, QPAD permits non-uniform ppc, and arbitrary 6D phase space initialization from hdf5 files (openPMD format). To resolve the plasma response for a given mode :math:`m`, it is highly recommended simulations use a sufficient number of particles per cell along the azimuthal direction  :math:`ppc_{\phi}\geq 8m`. 
...
.. math::
    n_b = \exp\left(-\frac{(x^2 + x_0^2)}{2\sigma^2}\right) \bigg[I_0(\alpha) + 2 \sum_{m=1}^{\infty} I_m(\alpha) \cos(m\phi)\bigg]
